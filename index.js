const authConfig = {
  "siteName": "GoIndex Plus", // site name
  "version": "C1.0.1", // program version. The user should not modify it manually
   ...
   * [id]: It can be the team disk id, subfolder id, or "root" (representing the root directory of the personal disk);
   * [name]: Displayed name
   ...
   * [Note] File links are not protected by default, which can facilitate direct-chain download/external playback;
  ...
  // Confirm that the cors purpose can be turned on
  "enable_cors_file_down": true,
  /**
   * The basic auth above already includes the function of global protection within the disk. Therefore, the password in the .password file is no longer authenticated by default;
   * If you still need to perform separate password verification in the .password file for certain directories on the basis of global authentication, set this option to true;
   * [Note] If the password verification of the .password file is enabled, every time the directory is listed, there will be an additional overhead of querying whether the .password file exists in the directory.
   */
